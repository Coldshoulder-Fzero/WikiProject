...
def test_unable_to_upload(client, wikiname, wikicontent):
    resp = client.post(
        "/upload", data={"wikiname": wikiname, "wikicontent": wikicontent})
    assert resp.status_code == 200
...
def test_upload_successful(client):
    ...
    with patch('flaskr.backend.Backend.upload', return_value=None):
        ...
        with patch("flaskr.backend.Backend.get_wiki_page", return_value=b"Some info."):
            upload_resp = client.post(
                "/upload",
                data={
                    "wikiname": "mywikiname",
                    "wikicontent": FileStorage(filename="myfile.txt", stream=b"Some info.")})
